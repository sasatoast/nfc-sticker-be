class Songs::SongsController < ApplicationController
  include Devise::Controllers::Helpers
  skip_before_action :authenticate_user!, only: [ :show, :show_share_ranking_by_artist_id ]
  # あとでここにcreate_song追加
  def show
    song_data = Song::SongPlayer.call(
      song_id: params[:id],
      share_id: params[:share_id],
      user_id: current_user&.id
    )
    if song_data
      render json: song_data, status: :ok
    else
      render json: { error: "Song not found" }, status: :not_found
    end
  end

  def show_received_song
    shared_song_data = Song::ReceivedSongFetcher.call(
      user_id: current_user&.id
    )
    if shared_song_data.present?
      render json: shared_song_data, status: :ok
    else
      render json: { shared_song_data: [], "message": "共有された曲はまだありません" }, status: :ok
    end
  end

  def register_sharable_song
    status, result = Song::SharableSongRegister.call(
      user_id: current_user&.id,
      song_id: params[:song_id],
      password: params[:password]
    )
    case status
    when :ok
      render json: { data: result }
    when :error
      render json: { error_code: result }, status: :unprocessable_entity
    end
  end

  def create_song
    status, result = Song::SecureSongCreator.call(
      name: params[:name],
      source_url: params[:source_url],
      picture_url: params[:picture_url],
      spotify_url: params[:spotify_url],
      apple_url: params[:apple_url],
      artist_name: params[:artist_name]
    )
    case status
    when :ok
      render json: { data: result }
    when :error
      render json: { data: result }, status: :unprocessable_entity
    end
  end

  def show_sharable_song
    songs_data = Song::SharableSongFetcher.call(
      user_id: current_user.id
    )
    if songs_data.present?
      render json: songs_data, status: :ok
    else
      render json: { "message": "共有できる曲はまだありません" }, status: :ok
    end
  end

  def show_shared_song
    songs_data = Song::SharedSongFetcher.call(
      share_id: current_user.share_id
    )
    if songs_data.present?
      render json: songs_data
    else
      render json: { "message": "まだ共有をしていません" }, status: :ok
    end
  end

  def show_share_ranking_by_artist_id
    data = Song::SharedRankingFetcher.call(
      artist_id: params[:artist_id]
    )
    render json: { data: data }, status: :ok
  end
end
